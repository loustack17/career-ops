...
9. If a cover letter is needed, load the report's `## Cover Letter Draft` and `modes/cover.md`. Generate final cover-letter PDF only after the user explicitly asks for cover mode and approves the drafted letter.
   - Save approved cover-letter PDFs under `/home/lou/Documents/Job/CVs/Cover Letters/{YYYY-MM-DD}/` unless the user explicitly asks for a different path.
...
- Do not use Playwright/Chromium for cover-letter PDF generation. Do not call `node generate-cover-letter.mjs` unless the user explicitly asks for the legacy HTML fallback.
