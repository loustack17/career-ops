...
2. **For each pending URL**:
   ...
   d. **Execute full auto-pipeline**: Evaluation A-F → Report .md → PDF (if score >= the resolved `auto_pdf_score_threshold`) → Tracker (`Evaluated` if score >= threshold, otherwise `SKIP`)
