...
2. **For each surviving pending URL**:
   ...
   e. **Execute full auto-pipeline**: Evaluation A-F → Report .md → PDF (if score >= `auto_pdf_score_threshold`) → Tracker (`Evaluated` if score >= threshold, otherwise `SKIP`). Read `modes/_custom.md` → Pipeline Rules, if it exists, and apply its override here. Default (if absent or silent): standard pipeline execution.
